...
const receiptSelect = `SELECT r.*, u.display_name AS created_by_name, f.display_name AS finalized_by_name
...
export async function listReceipts(search = "", status = "", page = 1) {
  ...
  const values = [
    pattern,
    ["OPEN", "FINALIZED"].includes(status) ? status : "",
  ];
  const where =
    "WHERE (r.reference ILIKE $1 OR r.supplier ILIKE $1) AND ($2='' OR r.status=$2)";
  ...
  const rows = await pool.query<ReceiptSummary>(
    ...
    LEFT JOIN LATERAL (SELECT coalesce(sum(quantity),0)::int AS units, count(DISTINCT sku) FILTER (WHERE quantity>0)::int AS sku_count FROM receipt_lines WHERE receipt_id=r.id) totals ON true
...
export async function getReceipt(id: string): Promise<ReceiptDetail> {
  ...
  return transaction(async (client) => {
    const result = await client.query<Receipt>(
      `${receiptSelect} WHERE r.id=$1 FOR SHARE OF r`,
      [id],
    );
